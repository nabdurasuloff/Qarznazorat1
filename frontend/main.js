...
function backendniIshgaTushirish() {
    ...
    // MUHIM: 'error' hodisasi tinglanmasa (masalan antivirus backend.exe ni
    // karantinga olgan bo'lsa), Node uni "uncaught exception" qilib otadi
    // va BUTUN dastur Electron'ning tushunarsiz xato oynasi bilan yopilib
    // qolardi. Endi xato faqat jurnalga yoziladi, dastur esa o'zining
    // tushunarli "Backend ishga tushmadi" ekranini ko'rsatadi.
    backendProcess.on('error', (e) => {
        console.error('[backend ishga tushmadi]', e && e.message ? e.message : e);
    });
    backendProcess.on('exit', (kod) => {
        if (kod !== 0) console.error(`[backend to'xtadi] chiqish kodi: ${kod}`);
    });
}
