...
# ══ ZAXIRA NUSXA (BACKUP) ══════════════════════════════════════════════
# MUHIM: butun huquqiy ish shu bitta baza faylida saqlanadi. Fayl
# buzilsa yoki qurilma yo'qolsa — hamma narsa yo'qoladi. Shu sabab
# tizim kuniga kamida bir marta AVTOMATIK zaxira nusxa oladi va oxirgi
# N kunlik nusxalarni saqlab turadi.
#
# Nusxa SQLite'ning O'Z backup API'si orqali olinadi — shunda baza
# ishlatilayotgan paytda ham (yozuv o'rtasida ham) BUTUN va shikastsiz
# nusxa chiqadi. Oddiy fayl ko'chirish buni kafolatlamaydi.

ZAXIRA_SAQLASH_KUN = 30
# Nusxalar soni ham cheklanadi: baza kattalashsa (portfel 50 mingdan ortiq
# qator) har nusxa o'nlab megabayt bo'ladi — telefon xotirasi to'lib
# qolmasligi uchun eng yangi N tasi saqlanadi, qolganlari o'chiriladi.
ZAXIRA_MAX_SONI = 20


def zaxira_papkasi():
    """Zaxira nusxalar saqlanadigan papka. Sozlamalarda hujjatlar papkasi
    ko'rsatilgan bo'lsa (masalan D:\\ diskda) — nusxalar o'sha yerda
    saqlanadi, ya'ni dastur papkasi bilan birga yo'qolib ketmaydi."""
    try:
        sozlama = (get_setting('hujjatlar_papkasi', '') or '').strip()
    except Exception:
        sozlama = ''
    asos = sozlama if sozlama and os.path.isdir(sozlama) else _app_dir()
    papka = os.path.join(asos, 'Zaxira nusxalar')
    os.makedirs(papka, exist_ok=True)
    return papka


def zaxira_yaratish(izoh='avtomatik'):
    """Bazaning to'liq zaxira nusxasini oladi va yo'lini qaytaradi."""
    if not os.path.exists(DB_PATH):
        return None
    papka = zaxira_papkasi()
    vaqt = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    nom = f"qarz_nazorat_{vaqt}_{re.sub(r'[^a-zA-Z0-9_]', '', izoh)[:20] or 'nusxa'}.db"
    maqsad = os.path.join(papka, nom)

    manba = sqlite3.connect(DB_PATH)
    nusxa = sqlite3.connect(maqsad)
    try:
        with nusxa:
            manba.backup(nusxa)   # SQLite'ning o'z, xavfsiz nusxalash usuli
    finally:
        nusxa.close()
        manba.close()

    try:
        set_setting('oxirgi_zaxira_sana', datetime.date.today().isoformat())
    except Exception:
        pass
    zaxira_eskilarini_tozalash()
    return maqsad


def zaxira_royxati():
    """Mavjud zaxira nusxalar ro'yxati (yangisidan eskisiga)."""
    papka = zaxira_papkasi()
    natija = []
    for nom in os.listdir(papka):
        if not nom.endswith('.db'):
            continue
        ...
        try:
            ...
            continue
        natija.append({
            'nomi': nom, 'yoli': yol, 'hajmi': st.st_size,
            'sana': datetime.datetime.fromtimestamp(st.st_mtime).strftime('%d.%m.%Y %H:%M'),
            'vaqt': st.st_mtime,
        })
    natija.sort(key=lambda r: r['vaqt'], reverse=True)
    return natija


def zaxira_eskilarini_tozalash(saqlash_kun=None, max_soni=None):
    """Eski nusxalarni ikki qoida bo'yicha tozalaydi:
      1) belgilangan kundan (standart 30) eski bo'lganlar o'chiriladi;
      2) soni chegaradan (standart 20) oshsa, eng eskilari o'chiriladi.
    Eng so'nggi 5 tasi har doim saqlanadi — dastur uzoq ishlatilmay
    turgan bo'lsa ham hech bo'lmasa bir nechta nusxa qolsin."""
    saqlash_kun = saqlash_kun or ZAXIRA_SAQLASH_KUN
    max_soni = max_soni or ZAXIRA_MAX_SONI
    chegara = datetime.datetime.now().timestamp() - saqlash_kun * 86400
    royxat = zaxira_royxati()   # yangisidan eskisiga
    ochiriladi = []
    for i, z in enumerate(royxat):
        if i < 5:
            continue
        if z['vaqt'] < chegara or i >= max_soni:
            ochiriladi.append(z['yoli'])
    for yol in ochiriladi:
        try:
            os.remove(yol)
        except OSError:
            pass
...
def zaxira_kerakmi():
    """Bugun hali zaxira olinmagan bo'lsa True qaytaradi.

    MUHIM: faqat sanaga ishonib bo'lmaydi. Agar nusxa fayli keyinchalik
    o'chirilgan yoki papka boshqa joyga ko'chirilgan bo'lsa, sana
    "bugun olingan" deb tursa-da, aslida hech qanday nusxa qolmagan
    bo'lardi va tizim yangisini olmay qo'yardi. Shu sabab HAQIQATDA
    fayl bor-yo'qligi ham tekshiriladi."""
    try:
        oxirgi = get_setting('oxirgi_zaxira_sana', '')
    except Exception:
        return True
    if oxirgi != datetime.date.today().isoformat():
        return True
    # Sana bugungi — lekin nusxa fayli haqiqatan turibdimi?
    try:
        return len(zaxira_royxati()) == 0
    except Exception:
        return True


def zaxiradan_tiklash(zaxira_yoli):
    """Bazani tanlangan zaxira nusxasidan tiklaydi.

    MUHIM: tiklashdan OLDIN joriy baza ham alohida nusxalanadi
    ('tiklashdan_oldin' izohi bilan) — agar noto'g'ri nusxa tanlangan
    bo'lsa, ortga qaytish imkoni qoladi."""
    if not zaxira_yoli or not os.path.exists(zaxira_yoli):
        raise FileNotFoundError('Zaxira fayli topilmadi')
    # Tanlangan fayl haqiqatan SQLite bazasimi — tekshiramiz.
    tekshir = sqlite3.connect(zaxira_yoli)
    try:
        tekshir.execute('SELECT COUNT(*) FROM sqlite_master').fetchone()
    finally:
        tekshir.close()

    try:
        zaxira_yaratish('tiklashdan_oldin')
    except Exception:
        pass

    manba = sqlite3.connect(zaxira_yoli)
    joriy = sqlite3.connect(DB_PATH)
    try:
        with joriy:
            manba.backup(joriy)
    finally:
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS xatlar (
        ...
        -- IQTISODIY SUD uchun MAJBURIY: davo ariza va talabnoma nusxalari
        -- taraflarga (javobgar, kafil, garov ta'minlovchi) yuborilganini
        -- tasdiqlovchi hujjat — pochta kvitansiyasi / topshirish xabarnomasi.
        davo_ariza_taraflarga_tasdiq_fayl TEXT,
        davo_ariza_taraflarga_sana TEXT,
        -- Sudga beriladigan rasmiy ma'lumotnomalar (tizim o'zi tayyorlaydi)
        sud_malumotnoma_qarzdorlik_fayl TEXT,
        sud_qoshimcha_malumotnoma_fayl TEXT,
        talabnoma_taraflarga_tasdiq_fayl TEXT,
        talabnoma_taraflarga_sana TEXT,
        sud_hujjatlar_ruxsat INTEGER DEFAULT 0,  -- 1 = barcha kerakli hujjatlar yuklanib, sudga yuborishga ruxsat berilgan
        sud_hujjatlar_ruxsat_sana TEXT,
    ...
    for col in ['davo_ariza_sana', 'davo_ariza_fayl_yoli', 'davo_ariza_holati',
                ...
                'davo_ariza_taraflarga_tasdiq_fayl', 'davo_ariza_taraflarga_sana',
                'talabnoma_taraflarga_tasdiq_fayl', 'talabnoma_taraflarga_sana',
    ...
    if 'sud_hujjatlar_ruxsat_sana' not in existing_cols:
        cur.execute('ALTER TABLE xatlar ADD COLUMN sud_hujjatlar_ruxsat_sana TEXT')
    ...
    # Biznes-hamroh jadvali avvalgi versiyada yaratilgan bo'lsa, yangi
    # ustunni qo'shib qo'yamiz (aks holda tasdiq fayli saqlanmay qolardi).
    try:
        bh_cols = {r['name'] for r in conn.execute("PRAGMA table_info(bh_murojaat)").fetchall()}
        if bh_cols and 'tasdiq_fayl' not in bh_cols:
            cur.execute('ALTER TABLE bh_murojaat ADD COLUMN tasdiq_fayl TEXT')
    except Exception:
        pass

    # Sug'urtadan undirish jadvali avvalgi versiyada (faqat MIB ishlari
    # uchun) yaratilgan bo'lsa — vafot etganlar bo'yicha ariza uchun
    # kerakli ustunlarni qo'shib qo'yamiz.
    try:
        su_cols = {r['name'] for r in conn.execute("PRAGMA table_info(sugurta_undirish)").fetchall()}
        if su_cols:
            yangi_ustunlar = {
                'tur': "TEXT DEFAULT 'mib'", 'vafot_id': 'INTEGER',
                'pasport_seriya': 'TEXT', 'pasport_raqam': 'TEXT',
                'guvohnoma_raqami': 'TEXT', 'guvohnoma_sanasi': 'TEXT',
                'vafot_sababi': 'TEXT', 'shartnoma_bandi': 'TEXT',
                'sugurta_shartnoma_raqami': 'TEXT', 'sugurta_shartnoma_sanasi': 'TEXT',
            }
            for ustun, tipi in yangi_ustunlar.items():
                if ustun not in su_cols:
                    cur.execute(f'ALTER TABLE sugurta_undirish ADD COLUMN {ustun} {tipi}')
            cur.execute("UPDATE sugurta_undirish SET tur='mib' WHERE tur IS NULL OR tur=''")
    except Exception:
        pass
    ...
    # ── BIZNES-HAMROH.UZ (elektron murojaat portali) ────────────────────
    # MUHIM: Davo arizalar endi SSPga qo'lda emas, biznes-hamroh.uz
    # portali orqali ELEKTRON yuboriladi. Bu jadval har bir anketa uchun
    # portalga kiritiladigan BARCHA ma'lumotlarni (portal shakli aynan
    # talab qiladigan tartibda) saqlaydi va murojaat holatini kuzatadi.
    cur.execute('''
    CREATE TABLE IF NOT EXISTS bh_murojaat (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        anketa_raqami TEXT,
        xat_id INTEGER,
        -- Portalga yuborish natijasida olingan ma'lumotlar
        murojaat_raqami TEXT,          -- masalan: 193/006526-ARZ
        yuborilgan_sana TEXT,
        ijro_muddati TEXT,
        yuborilgan_tashkilot TEXT,     -- hududiy boshqarma nomi
        sud_nomi TEXT,
        -- Holat kuzatuvi
        holati TEXT DEFAULT 'tayyorlanmoqda',
        holat_sana TEXT,
        natija_izoh TEXT,
        tasdiq_fayl TEXT,              -- qabul qilinganini tasdiqlovchi PDF
        -- Javobgar (portal "Javobgar ma'lumotlari" oynasi)
        javobgar_ishtirok_turi TEXT,
        asos_hujjat_turi TEXT,
        asos_hujjat_sanasi TEXT,
        asos_hujjat_raqami TEXT,
        -- Kredit shartnoma (portal "Kredit ma'lumotlari" oynasi)
        shartnoma_raqami TEXT,
        shartnoma_sanasi TEXT,
        shartnoma_foizi TEXT,
        shartnoma_muddati TEXT,
        valyuta TEXT,
        shartnoma_summasi REAL,
        -- Qarz bo'limi
        qarz_hisoblangan_sana TEXT,
        davo_summasi REAL,
        joriy_asosiy_qarz REAL,
        joriy_hisoblangan_foiz REAL,
        mo_asosiy_qarz REAL,           -- asosiy qarz bo'yicha muddati o'tgan qarzdorlik
        mo_asosiy_foiz REAL,           -- muddati o'tgan asosiy qarzga hisoblangan foiz
        mo_foiz_tolovi REAL,           -- muddati o'tgan foiz to'lovi
        yaratilgan_sana TEXT
    )
    ''')

    cur.execute('''
    CREATE TABLE IF NOT EXISTS bh_ilovalar (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        murojaat_id INTEGER,
        ilova_turi TEXT,
        fayl_yoli TEXT,
    ...
    # MUHIM: bu bo'limning ASOSIY MANTIG'I — MIB (Majburiy ijro byurosi)
    # tomonidan CHIQARILGAN hujjatlar (masalan "undirib bo'lmaganligi
    # to'g'risidagi dalolatnoma", "ijro hujjatini qaytarish to'g'risidagi
    # qaror" va h.k.). Aynan shu hujjat sug'urta kompaniyasidan qarzni
    # undirish uchun ASOS bo'ladi. Shu sabab ro'yxatga BARCHA MIB ishlari
    # (faol va yakunlangan) tushadi.
    cur.execute('''
    CREATE TABLE IF NOT EXISTS sugurta_undirish (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        anketa_raqami TEXT,
        xat_id INTEGER,
        -- Ish turi: 'mib'   — MIB qarori asosida (muddatidan oldin undirish)
        --           'vafot' — sug'urtalangan shaxsning vafoti munosabati bilan
        tur TEXT DEFAULT 'mib',
        vafot_id INTEGER,
        -- Vafot arizasi uchun QO'LDA kiritiladigan maydonlar (tizimda yo'q)
        pasport_seriya TEXT,
        pasport_raqam TEXT,
        guvohnoma_raqami TEXT,
        guvohnoma_sanasi TEXT,
        vafot_sababi TEXT,
        shartnoma_bandi TEXT,
        sugurta_shartnoma_raqami TEXT,
        sugurta_shartnoma_sanasi TEXT,
        -- Sug'urta polisi ma'lumotlari
        sugurta_kompaniya TEXT,
        polis_raqami TEXT,
        polis_sanasi TEXT,
        polis_fayl TEXT,
        -- ASOS: MIB chiqargan hujjat (nomini xodim o'zi yozadi)
        mib_asos_hujjat_nomi TEXT,
        mib_asos_hujjat_sanasi TEXT,
        mib_asos_hujjat_raqami TEXT,
        mib_asos_fayl TEXT,
        -- Sug'urta kompaniyasiga yuborilgan ariza
        ariza_raqami TEXT,
        ariza_sana TEXT,
        ariza_fayl TEXT,
        talab_summasi REAL,
        -- Holat kuzatuvi: yuborildi -> tolandi / rad_etildi
        holati TEXT DEFAULT 'tayyorlanmoqda',
        holat_sana TEXT,
        natija_izoh TEXT,
        natija_fayl TEXT,
    ...
    cur.execute('''
    CREATE TABLE IF NOT EXISTS sugurta_hujjatlar (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        undirish_id INTEGER,
        hujjat_nomi TEXT,
        fayl_yoli TEXT,
    ...
    cur.execute('''
    CREATE TABLE IF NOT EXISTS sugurta_kompaniyalar (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nomi TEXT UNIQUE,
        faol INTEGER DEFAULT 1
    )
    ...
    defaults = {
        ...
        # Sug'urtadan undirish: ariza yuborilgandan keyin javob kutish
        # muddati (kun) — o'tib ketsa ro'yxatda ogohlantirish chiqadi.
        'sugurta_undirish_muddat_kun': '30',
        ...
        # --- Biznes-hamroh.uz portali uchun ---
        'bh_mfo': '00382',
        'bh_masul_xodim': '',
        'bh_masul_tel': '',
        'bh_hududiy_boshqarma': "Sirdaryo viloyati hududiy boshqarmasi",
        'bh_arizachi_turi': 'Yuridik shaxs',
    ...
    # ── TEZLIK INDEKSLARI ────────────────────────────────────────────
    # MUHIM: bazada faqat BITTA indeks (port_kod) bor edi. Natijada
    # eng ko'p ishlatiladigan qidiruvlar 55 000 qatorli `portfel`
    # jadvalini HAR SAFAR boshidan oxirigacha o'qib chiqardi. Bu ayniqsa
    # sezilarli edi: ommaviy xat yaratishda har bir anketa uchun alohida
    # to'liq skanerlash bo'lardi (1000 ta xat = 1000 marta 55 000 qator).
    # Quyidagi indekslar bazani sezilarli tezlashtiradi va hech qanday
    # ma'lumotni o'zgartirmaydi (faqat qidiruvni tezlashtiradi).
    for indeks_sql in [
        'CREATE INDEX IF NOT EXISTS idx_portfel_anketa ON portfel(anketa_raqami)',
        'CREATE INDEX IF NOT EXISTS idx_portfel_faol_dpd ON portfel(faol, dpd_max)',
        'CREATE INDEX IF NOT EXISTS idx_portfel_pinfl ON portfel(pinfl)',
        'CREATE INDEX IF NOT EXISTS idx_portfel_stir ON portfel(stir)',
        'CREATE INDEX IF NOT EXISTS idx_xatlar_portfel ON xatlar(portfel_id)',
        'CREATE INDEX IF NOT EXISTS idx_xatlar_anketa ON xatlar(anketa_raqami)',
        'CREATE INDEX IF NOT EXISTS idx_xatlar_holat ON xatlar(holat)',
        'CREATE INDEX IF NOT EXISTS idx_xatlar_sud_holati ON xatlar(sud_holati)',
        'CREATE INDEX IF NOT EXISTS idx_xatlar_mib_holati ON xatlar(mib_holati)',
        'CREATE INDEX IF NOT EXISTS idx_vafot_anketa ON vafot_etganlar(anketa_raqami)',
        'CREATE INDEX IF NOT EXISTS idx_mib_amallar_xat ON mib_amallar(xat_id)',
        'CREATE INDEX IF NOT EXISTS idx_tolovlar_anketa ON tolovlar(anketa_raqami)',
        'CREATE INDEX IF NOT EXISTS idx_mijozlar_pinfl ON mijozlar(pinfl)',
        'CREATE INDEX IF NOT EXISTS idx_mijozlar_stir ON mijozlar(stir)',
    ]:
        try:
            cur.execute(indeks_sql)
        except Exception:
            pass  # ustun mavjud bo'lmagan eski bazada — indekssiz davom etadi
...
# RAQAMLI sozlamalar: kalit -> standart (zaxira) qiymat.
# MUHIM: bu sozlamalar butun tizim bo'ylab int()/float() bilan o'qiladi.
# Ilgari ular matn sifatida, HECH QANDAY tekshiruvsiz saqlanardi —
# natijada foydalanuvchi Sozlamalarda bironta raqamli maydonni BO'SHATIB
# saqlasa yoki "45 kun" deb yozsa, int('') xato berib, BOSH SAHIFA,
# TALABNOMA, CHORA KO'RISH kabi ekranlar butunlay ochilmay qolardi
# (foydalanuvchi esa faqat "Xato: API xato: 500" ni ko'rardi va sababini
# topa olmasdi). Endi noto'g'ri qiymat o'qishda standart qiymat bilan
# almashtiriladi — ekran hech qachon "o'lmaydi".
RAQAMLI_SOZLAMALAR = {
    'dpd_chegara_kun': 45, 'chora_ogohlantirish_dpd_kun': 45,
    'eslatma_muddati_kun': 3, 'tolov_muddati_kun': 10,
    'davo_ariza_muddati_kun': 5, 'davo_ariza_muddati_iqtisodiy_kun': 15,
    'sud_topshirish_muddati_kun': 5, 'sud_kiritilmadi_qayta_tekshirish_kun': 30,
    'chora_ogohlantirish_oy': 2, 'chora_davo_ariza_dpd_kun': 60,
    'chora_davo_ariza_oy': 1, 'mib_toxtatish_dpd_chegara': 30,
    'sugurta_undirish_muddat_kun': 30, 'bxm_miqdori': 375000,
    'pochta_xarajati_standart': 41200, 'mib_harakatsizlik_kun': 30,
}


def sozlama_raqami(qiymat, standart):
    """Sozlama qiymatini raqamga aylantiradi. Bo'sh, noto'g'ri yoki
    bo'shliqli ("1 200 000") qiymatlar ham to'g'ri ishlanadi."""
    if qiymat is None:
        return standart
    matn = str(qiymat).replace(' ', '').replace(' ', '').replace(',', '.').strip()
    if not matn:
        return standart
    try:
        son = float(matn)
        ...
        return standart
    return son if isinstance(standart, float) else int(son)
...
def get_all_settings():
    ...
    conn.close()
    natija = {r['kalit']: r['qiymat'] for r in rows}
    # Raqamli sozlamalarni xavfsiz holatga keltiramiz (yuqoridagi izohga qarang)
    for kalit, standart in RAQAMLI_SOZLAMALAR.items():
        if kalit in natija:
            natija[kalit] = sozlama_raqami(natija[kalit], standart)
    return natija
...
def mijoz_kalitlari_toplami():
    """Mijozlar bazasidagi BARCHA (turi, kalit) juftliklarini BITTA so'rovda
    to'plam qilib qaytaradi.

    MUHIM (tezlik): ro'yxat ekranlarida har bir qator uchun `find_mijoz()`
    chaqirilardi, u esa HAR SAFAR yangi baza ulanishini ochib-yopardi.
    6 000+ qatorli ro'yxatda bu 20 000 gacha ulanish degani edi va
    Talabnoma ro'yxati 5 sekundgacha ochilardi. Endi kalitlar bir marta
    o'qib olinib, xotirada tekshiriladi."""
    conn = get_conn()
    rows = conn.execute('SELECT turi, kalit FROM mijozlar').fetchall()
    conn.close()
    return {(r['turi'], str(r['kalit']).strip()) for r in rows}
...
def get_tahlil_tarixi(oxirgi_n_kun=90):
    """Portfel tendensiyasi grafigi uchun OXIRGI N kunlik suratlar.

    MUHIM TUZATISH: ilgari `ORDER BY sana ASC LIMIT ?` yozilgan edi — bu
    esa OXIRGI emas, ENG ESKI N ta yozuvni qaytarardi. Natijada 90 kundan
    keyin Bosh sahifa va Tahlil bo'limidagi tendensiya grafigi birinchi
    90 kunda "muzlab" qolib, joriy holatni umuman ko'rsatmay qo'yardi.
    Endi oxirgi N ta olinadi va grafik uchun eskidan yangiga tartiblanadi."""
    conn = get_conn()
    rows = conn.execute(
        'SELECT * FROM tahlil_tarixi ORDER BY sana DESC LIMIT ?', (oxirgi_n_kun,)
    ).fetchall()
    conn.close()
...
def get_portfel_by_anketa(anketa_raqami):
    """Anketa raqami bo'yicha portfel qatorlarini topadi.

    MUHIM XAVFSIZLIK TUZATISHI: ilgari bu yerda `LIKE '%raqam%'` ishlatilardi,
    ya'ni anketa raqami boshqa anketa raqamining ICHIDA uchrasa ham, u ham
    "topilgan" hisoblanardi. Haqiqiy portfelda tekshirildi: 300 ta anketadan
    70 tasi (23%!) shu yo'l bilan BIR NECHTA mijozga mos kelardi. Chaqiruvchi
    tomonlar esa odatda birinchi natijani (`[0]`) oladi — natijada, masalan
    "1234" anketasi uchun xat yoki Davo ariza tayyorlanganda, hujjatga
    "91234" anketasidagi BOShQA MIJOZning ismi, manzili va qarz summasi
    tushib qolishi mumkin edi. Endi faqat ANIQ moslik qidiriladi.

    Bo'sh natija bo'lsa — bo'shliqlar olib tashlangan holda yana bir marta
    urinib ko'riladi (Excel'dan kelgan raqamlarda ortiqcha bo'shliq bo'lishi
    mumkin), lekin baribir ANIQ moslik asosida.
    """
    kalit = str(anketa_raqami or '').strip()
    if not kalit:
        return []
    conn = get_conn()
    rows = conn.execute(
        'SELECT * FROM portfel WHERE anketa_raqami = ?', (kalit,)
    ).fetchall()
    if not rows:
        rows = conn.execute(
            "SELECT * FROM portfel WHERE TRIM(anketa_raqami) = ?", (kalit,)
        ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def iqtisodiy_sudmi(mijoz_turi):
    """Ish IQTISODIY sudda ko'riladimi? Yuridik shaxs va YaTT — iqtisodiy
    sudda, jismoniy shaxs — fuqarolik sudida ko'riladi."""
    # Eski yozuvlarda portfeldagi xom qiymat ('LE') saqlangan bo'lishi
    # mumkin — shu sabab u ham hisobga olinadi.
    return str(mijoz_turi or '').strip().lower() in ('yuridik', 'yatt', 'le')


# IQTISODIY SUDGA topshiriladigan ishlarda MAJBURIY qo'shimcha hujjatlar.
# Sabab: iqtisodiy sudda davo arizani sudga berishdan OLDIN uning nusxasi
# (va unga ilova qilinadigan hujjatlar) barcha taraflarga — javobgarga,
# kafilga, garov ta'minlovchisiga — yuborilishi shart, va sudga bu
# yuborilganini TASDIQLOVCHI hujjat (pochta kvitansiyasi yoki topshirish
# xabarnomasi) ilova qilinadi. Talabnoma (da'vogacha murojaat) bo'yicha
# ham xuddi shunday tasdiq talab qilinadi. Bu hujjatlar bo'lmasa, sud
# davo arizani harakatsiz qoldiradi yoki qaytarib yuboradi — shu sabab
# tizim ularsiz "sudga topshirishga ruxsat" bermaydi.
IQTISODIY_SUD_MAJBURIY_HUJJATLAR = [
    ('davo_ariza_taraflarga_tasdiq_fayl', 'Davo ariza taraflarga yuborilgani tasdig\'i'),
    ('talabnoma_taraflarga_tasdiq_fayl', 'Talabnoma taraflarga yuborilgani tasdig\'i'),
]

# Sud yig'ma jildiga QO'LDA yuklanishi mumkin bo'lgan hujjat maydonlari
SUD_YUKLANADIGAN_HUJJATLAR = (
    'kredit_shartnoma_fayl', 'kafillik_shartnoma_fayl', 'garov_shartnoma_fayl',
    'bank_baholash_fayl', 'sud_malumotnoma_topshirishda_fayl', 'sud_malumotnoma_kun_fayl',
    'davo_ariza_taraflarga_tasdiq_fayl', 'talabnoma_taraflarga_tasdiq_fayl',
    'sud_malumotnoma_qarzdorlik_fayl', 'sud_qoshimcha_malumotnoma_fayl',
)

# Tasdiq hujjati -> yuborilgan sana maydoni
TARAFLARGA_SANA_MAYDONI = {
    'davo_ariza_taraflarga_tasdiq_fayl': 'davo_ariza_taraflarga_sana',
    'talabnoma_taraflarga_tasdiq_fayl': 'talabnoma_taraflarga_sana',
}


def sud_majburiy_hujjatlar_royxati(xat):
    """Shu ANIQ ish uchun sudga topshirishdan oldin MAJBURIY bo'lgan barcha
    hujjatlar ro'yxati: kredit hujjatlari + (iqtisodiy sud bo'lsa) taraflarga
    yuborilganini tasdiqlovchi hujjatlar."""
    royxat = list(sud_kredit_hujjatlar_royxati(xat.get('davo_ariza_turi')))
    if iqtisodiy_sudmi(xat.get('mijoz_turi')):
        royxat.extend(IQTISODIY_SUD_MAJBURIY_HUJJATLAR)
    return royxat
...
def sud_hujjat_saqlash(xat_id, maydon_nomi, fayl_yoli, yuborilgan_sana=None):
    ...
    if maydon_nomi not in SUD_YUKLANADIGAN_HUJJATLAR:
        raise ValueError("Noto'g'ri maydon nomi")
    conn = get_conn()
    conn.execute(f'UPDATE xatlar SET {maydon_nomi}=? WHERE id=?', (fayl_yoli, xat_id))
    # Taraflarga yuborilgan sana (kvitansiya sanasi) ham saqlanadi
    if maydon_nomi in TARAFLARGA_SANA_MAYDONI and yuborilgan_sana:
        conn.execute(f'UPDATE xatlar SET {TARAFLARGA_SANA_MAYDONI[maydon_nomi]}=? WHERE id=?',
                     (yuborilgan_sana, xat_id))
...
def sud_hujjatlar_toliqmi(xat):
    ...
    kerakli = sud_majburiy_hujjatlar_royxati(xat)
...
def sud_hujjatlar_ruxsat_berish(xat_id):
    conn = get_conn()
    conn.execute('UPDATE xatlar SET sud_hujjatlar_ruxsat=1, sud_hujjatlar_ruxsat_sana=? WHERE id=?',
                 (datetime.datetime.now().strftime('%d.%m.%Y'), xat_id))
    conn.commit()
    conn.close()
...
# ══ BIZNES-HAMROH.UZ ═══════════════════════════════════════════════════
# Portalning o'z ro'yxatlaridan olingan aniq qiymatlar.

BH_HOLAT_NOMLARI = {
    'tayyorlanmoqda': "📝 Tayyorlanmoqda",
    'yuborildi': "📤 Portalga yuborildi",
    'javob_kutilmoqda': "⏳ Javob kutilmoqda",
    'qabul_qilindi': "✓ Qabul qilindi",
    'rad_etildi': "✕ Rad etildi",
}

# Portal "ИЛОВА" oynasidagi ilova turlari (aynan portaldagi tartibda).
BH_ILOVA_TURLARI = [
    "SSPga a'zolik guvohnomasi va a'zolik shartnomasi nusxalari.",
    "Kredit shartnomasi va unga 1-ilova nusxasi.",
    "Garov shartnomasi nusxasi.",
    "Tex. pasport nusxasi.",
    "Talabnoma xatlari nusxasi.",
    "Javobgarning davlat ro'yxatidan o'tganligi haqidagi guvohnoma.",
    "Javobgarning pasport nusxasi.",
    "Yuridik shaxsning davlat ro'yhatidan o'tganligi haqidagi guvohnoma.",
    "Qarzdorlik bo'yicha ma'lumotnoma.",
    "SSPga murojatda murojatchining ilova fayli.",
]

# Portal "Ishtirok turi" ro'yxati.
BH_ISHTIROK_TURLARI = ["Qarzdor", "Kafil", "Garov ta'minlovchi", "Uchinchi shaxs"]


def bh_murojaat_olish(anketa_raqami):
    """Berilgan anketa uchun biznes-hamroh murojaat yozuvini qaytaradi
    (yo'q bo'lsa None)."""
    conn = get_conn()
    row = conn.execute(
        'SELECT * FROM bh_murojaat WHERE anketa_raqami=? ORDER BY id DESC LIMIT 1',
        (anketa_raqami,)).fetchone()
    conn.close()
    return dict(row) if row else None


def bh_murojaat_saqlash(anketa_raqami, xat_id=None, **fields):
    """Murojaat yozuvini yaratadi yoki mavjudini yangilaydi. Faqat
    uzatilgan maydonlar o'zgaradi — qolganlari tegilmaydi."""
    mavjud = bh_murojaat_olish(anketa_raqami)
    conn = get_conn()
    if mavjud:
        if fields:
            set_clause = ', '.join(f'{k}=?' for k in fields)
            conn.execute(f'UPDATE bh_murojaat SET {set_clause} WHERE id=?',
                         list(fields.values()) + [mavjud['id']])
        murojaat_id = mavjud['id']
    else:
        fields['anketa_raqami'] = anketa_raqami
        fields['xat_id'] = xat_id
        fields.setdefault('holati', 'tayyorlanmoqda')
        fields['yaratilgan_sana'] = datetime.datetime.now().isoformat()
        cols = ', '.join(fields)
        placeholders = ', '.join(['?'] * len(fields))
        cur = conn.execute(f'INSERT INTO bh_murojaat ({cols}) VALUES ({placeholders})',
                           list(fields.values()))
...
def bh_holat_ozgartirish(anketa_raqami, yangi_holat, izoh=None):
    bh_murojaat_saqlash(anketa_raqami, holati=yangi_holat,
                        holat_sana=datetime.datetime.now().strftime('%d.%m.%Y'),
                        natija_izoh=izoh or '')


def bh_murojaatlar_royxati(holat=None):
    """Barcha biznes-hamroh murojaatlari (ixtiyoriy holat bo'yicha filtr)."""
    conn = get_conn()
    if holat:
        rows = conn.execute('SELECT * FROM bh_murojaat WHERE holati=? ORDER BY id DESC',
                            (holat,)).fetchall()
    else:
        rows = conn.execute('SELECT * FROM bh_murojaat ORDER BY id DESC').fetchall()
    conn.close()
    return [dict(r) for r in rows]


def bh_ilova_qoshish(murojaat_id, ilova_turi, fayl_yoli):
    conn = get_conn()
    conn.execute('''INSERT INTO bh_ilovalar (murojaat_id, ilova_turi, fayl_yoli, yuklangan_sana)
                    VALUES (?, ?, ?, ?)''',
                 (murojaat_id, ilova_turi, fayl_yoli, datetime.datetime.now().isoformat()))
    conn.commit()
    conn.close()


def bh_ilovalar_royxati(murojaat_id):
    conn = get_conn()
    rows = conn.execute('SELECT * FROM bh_ilovalar WHERE murojaat_id=? ORDER BY id ASC',
                        (murojaat_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def bh_ilova_ochirish(ilova_id):
    conn = get_conn()
    conn.execute('DELETE FROM bh_ilovalar WHERE id=?', (ilova_id,))
    conn.commit()
    conn.close()
...
def get_bh_royxati():
    ...
    allaqachon yuborilgan) ishlar ro'yxati.

    MUHIM: portal SSP o'rnini egallagani uchun, bu ro'yxatga Davo ariza
    TAYYORLANGAN, lekin hali sudga topshirilmagan ishlar tushadi.
    Har bir ish uchun portal murojaati holati ham birga qaytariladi.

    MUHIM (tuzatildi): ALLAQACHON TASDIQLANGAN (davo_ariza_holati=
    'olib_kelindi') va portalga umuman yuborilmagan ishlar bu ro'yxatga
    TUSHMAYDI. Bular — portal joriy etilishidan OLDIN, eski yo'l bilan
    SSPdan (Palatadan) tasdiqlanib qaytgan arizalar. Ilgari ular
    ro'yxatda "📝 Tayyorlanmoqda" bo'lib turardi va xodim allaqachon
    tasdiqlangan arizani portalga QAYTA yuborib yuborishi mumkin edi.
    Ularning keyingi qadami — portal emas, SUDGA TOPSHIRISH.

    Portalda yozuvi BOR ishlar (yuborilgan, qabul qilingan yoki rad
    etilgan) esa tasdiqlangandan keyin ham ro'yxatda qoladi — chunki
    ularning portal tarixi va murojaat raqami shu yerda ko'rinishi
    kerak."""
    conn = get_conn()
    rows = conn.execute('''
        SELECT * FROM xatlar
    ...
    for r in rows:
        x = dict(r)
        anketa = x['anketa_raqami']
        if anketa in korilgan:
            continue  # bitta anketa uchun faqat eng so'nggi sikl
        murojaat = bh_murojaat_olish(anketa)
        # Eski yo'l bilan tasdiqlangan, portalga aloqasi yo'q ish —
        # bu yerda qiladigan ish qolmagan.
        if x.get('davo_ariza_holati') == 'olib_kelindi' and not murojaat:
            continue
        korilgan.add(anketa)
        natija.append({'xat': x, 'murojaat': murojaat})
    return natija


# ══ SUG'URTADAN UNDIRISH ═══════════════════════════════════════════════
# MIB chiqargan hujjat (dalolatnoma/qaror) asosida sug'urta kompaniyasidan
# qarzni undirish jarayoni. 3 bosqich:
#     Ariza yuborildi  →  To'landi  |  Rad etildi

SUGURTA_HOLAT_NOMLARI = {
    'tayyorlanmoqda': "📝 Tayyorlanmoqda",
    'yuborildi': "📤 Ariza yuborildi",
    'tolandi': "✓ To'landi",
    'rad_etildi': "✕ Rad etildi",
}

# Sug'urta kompaniyalari ro'yxati foydalanuvchi tomonidan Sozlamalarda
# to'ldiriladi (MIB harakat turlari kabi) — tizimda qattiq belgilangan
# ro'yxat yo'q, chunki har bankda o'z sheriklari bo'ladi.


def sugurta_kompaniya_qoshish(nomi):
    conn = get_conn()
    try:
        conn.execute('INSERT OR IGNORE INTO sugurta_kompaniyalar (nomi, faol) VALUES (?, 1)',
                     (nomi.strip(),))
        conn.commit()
    finally:
        conn.close()
...
def sugurta_kompaniyalar_royxati(faqat_faol=True):
    conn = get_conn()
    if faqat_faol:
        rows = conn.execute('SELECT * FROM sugurta_kompaniyalar WHERE faol=1 ORDER BY nomi').fetchall()
    else:
        rows = conn.execute('SELECT * FROM sugurta_kompaniyalar ORDER BY nomi').fetchall()
    conn.close()
    return [dict(r) for r in rows]


def sugurta_kompaniya_ochirish(kompaniya_id):
    conn = get_conn()
    conn.execute('DELETE FROM sugurta_kompaniyalar WHERE id=?', (kompaniya_id,))
    conn.commit()
    conn.close()


def sugurta_undirish_olish(anketa_raqami, tur='mib'):
    """Berilgan anketa va ISH TURI uchun sug'urta yozuvi (yo'q bo'lsa None).
    tur: 'mib' — MIB qarori asosida, 'vafot' — vafot etgan mijoz bo'yicha.
    Bitta mijozda ikkala jarayon ham bo'lishi mumkin, shuning uchun ular
    alohida yozuvlarda saqlanadi."""
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM sugurta_undirish WHERE anketa_raqami=? AND COALESCE(tur,'mib')=? "
        "ORDER BY id DESC LIMIT 1", (anketa_raqami, tur)).fetchone()
    conn.close()
    return dict(row) if row else None


def sugurta_undirish_saqlash(anketa_raqami, xat_id=None, tur='mib', **fields):
    """Yozuvni yaratadi yoki mavjudini yangilaydi. Faqat uzatilgan
    maydonlar o'zgaradi — qolganlari tegilmaydi."""
    mavjud = sugurta_undirish_olish(anketa_raqami, tur)
    conn = get_conn()
    if mavjud:
        if fields:
            set_clause = ', '.join(f'{k}=?' for k in fields)
            conn.execute(f'UPDATE sugurta_undirish SET {set_clause} WHERE id=?',
                         list(fields.values()) + [mavjud['id']])
        undirish_id = mavjud['id']
    else:
        fields['anketa_raqami'] = anketa_raqami
        fields['xat_id'] = xat_id
        fields['tur'] = tur
        ...
        cols = ', '.join(fields)
        placeholders = ', '.join(['?'] * len(fields))
        cur = conn.execute(f'INSERT INTO sugurta_undirish ({cols}) VALUES ({placeholders})',
                           list(fields.values()))
        undirish_id = cur.lastrowid
    conn.commit()
    conn.close()
    return undirish_id


def sugurta_holat_ozgartirish(anketa_raqami, yangi_holat, izoh=None, tur='mib', **qoshimcha):
    qoshimcha['holati'] = yangi_holat
    qoshimcha['holat_sana'] = datetime.datetime.now().strftime('%d.%m.%Y')
    qoshimcha['natija_izoh'] = izoh or ''
    sugurta_undirish_saqlash(anketa_raqami, tur=tur, **qoshimcha)


def sugurta_hujjat_qoshish(undirish_id, hujjat_nomi, fayl_yoli):
    conn = get_conn()
    conn.execute('''INSERT INTO sugurta_hujjatlar (undirish_id, hujjat_nomi, fayl_yoli, yuklangan_sana)
                    VALUES (?, ?, ?, ?)''',
                 (undirish_id, hujjat_nomi, fayl_yoli, datetime.datetime.now().isoformat()))
    conn.commit()
    conn.close()


def sugurta_hujjatlar_royxati(undirish_id):
    conn = get_conn()
    rows = conn.execute('SELECT * FROM sugurta_hujjatlar WHERE undirish_id=? ORDER BY id ASC',
                        (undirish_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def sugurta_hujjat_ochirish(hujjat_id):
    conn = get_conn()
    conn.execute('DELETE FROM sugurta_hujjatlar WHERE id=?', (hujjat_id,))
    conn.commit()
    conn.close()


def get_sugurta_undirish_royxati():
    """Sug'urtadan undirish bo'limining ro'yxati.

    MUHIM (asosiy shart): sug'urtadan pul undirish FAQAT kredit qarzini
    MUDDATIDAN OLDIN undirish uchun chiqarilgan sud qarorlari bo'yicha
    hisoblanadi — sug'urta hodisasi aynan shunday qaror asosida yuzaga
    keladi. Shuning uchun ro'yxatga MIBga o'tkazilgan ishlarning
    HAMMASI emas, balki Davo arizasi 'muddatidan oldin undirish' turida
    bo'lganlari tushadi.

    Istisno: agar bir ish bo'yicha sug'urta undirish ALLAQACHON
    boshlangan bo'lsa (masalan tizimga eski ish sifatida kiritilgan va
    ariza turi qayd etilmagan), u ro'yxatdan yo'qolib qolmaydi."""
    conn = get_conn()
    rows = conn.execute('''
        SELECT * FROM xatlar
        WHERE mib_holati='otkazildi'
          AND (arxivlangan IS NULL OR arxivlangan = 0)
        ORDER BY mib_otkazilgan_sana DESC, id DESC
    ''').fetchall()
    conn.close()
    ...
    for r in rows:
        ...
        anketa = x['anketa_raqami']
        if anketa in korilgan:
            continue  # bitta anketa uchun faqat eng so'nggi MIB sikli
        undirish = sugurta_undirish_olish(anketa)
        muddatidan_oldin = (x.get('davo_ariza_turi') or '') == 'muddatidan_oldin'
        if not muddatidan_oldin and not undirish:
            continue
        korilgan.add(anketa)
        natija.append({'xat': x, 'undirish': undirish,
                       'muddatidan_oldin': muddatidan_oldin})
    return natija


def get_sugurta_vafot_royxati():
    """Sug'urtadan undirish bo'limining "Vafot etganlar" ro'yxati.

    MUHIM: bu yerga "Vafot etganlar" bo'limiga kiritilgan BARCHA
    mijozlar AVTOMATIK tushadi — chunki sug'urtalangan shaxsning vafoti
    o'z-o'zidan sug'urta hodisasi hisoblanadi (MIB qarori talab
    qilinmaydi). Har biri uchun sug'urta jarayoni holati birga
    qaytariladi."""
    conn = get_conn()
    rows = conn.execute(
        'SELECT * FROM vafot_etganlar ORDER BY vafot_sanasi DESC, id DESC').fetchall()
    conn.close()

    natija = []
    for r in rows:
        v = dict(r)
        natija.append({'vafot': v,
                       'undirish': sugurta_undirish_olish(v['anketa_raqami'], 'vafot')})
    return natija
